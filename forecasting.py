...
# from tensorflow import ExponentialDecay
import datetime
import os
import gc
import random
        ...
        for code in codes:
            if code not in issuer:
                ...
                for lookback in lookbacks:
                    ...
                    if os.path.exists(checkfiles) == False or out_type != 0:
                        print("\nmasukkk20")
                        if (self.output_type == 0):
                            ...
                            model.compile(
                                loss="mse",
                                optimizer=tf.keras.optimizers.Adam(learning_rate=lr_schedule),  # Note the comma separating the arguments.
                                metrics=[
                            ...
                            print("\nmasukkk1")
                            model_path = f'results/models/{self.model}_{type_dataset}_BBCA.h5'
                        ...
                        if(self.output_type == 0):
                            filename_train = f'{code_stock}_{lookback}_train'
                            filename_test = f'{code_stock}_{lookback}_test'
                        else:
                            filename_train = f'TL_{code_stock}_USING_{self.model}_{type_dataset}_BBCA_Train'
                            filename_test = f'TL_{code_stock}_USING_{self.model}_{type_dataset}_BBCA_Test'
                        ...
                        if(self.output_type == 0):
                            savepath=f'results/models/{self.model}_{type_dataset}_{code_stock}.h5'
                            model.save(savepath)
...
if __name__ == '__main__':
    ...
    out_type = int(input("Masukkan Jenis Tipenya: "))
    parser.add_argument("--output_type", type=int, default=out_type)
    parser.add_argument("--code", type=str, default='BBRI',
